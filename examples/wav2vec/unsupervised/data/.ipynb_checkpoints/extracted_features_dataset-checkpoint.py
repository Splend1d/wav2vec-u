...
class ExtractedFeaturesDataset(FairseqDataset):
    def __init__(
        ...
        # hydra multigpu cannot handle leftover batch for training, slice data to multiple of 128
        #if labels is not None:
        n_data = len(self.sizes) // 128 * 128
        print(n_data)

        self.sizes = self.sizes[:n_data]
        self.offsets = self.offsets[:n_data]
        # end of patch
