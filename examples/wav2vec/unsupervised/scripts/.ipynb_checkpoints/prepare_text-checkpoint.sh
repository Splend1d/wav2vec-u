...
# +
mkdir -p $target_dir
echo "python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/normalize_and_filter_text.py --lang $lg --fasttext-model $lid_path < $text_path | grep -v '\-\-\-' >! $target_dir/lm.upper.lid.txt"
...
echo "python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py  --dataset-impl mmap --trainpref $target_dir/lm.upper.lid.txt --only-source --destdir $target_dir --thresholdsrc 2 --padding-factor 1 --dict-onlycut -f1 -d' ' $target_dir/dict.txt | grep -v -x '[[:punct:]]*' | grep -Pv '\d\d\d\d\d+' >! $target_dir/words.txt"
python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --workers 4 --dataset-impl mmap --trainpref $target_dir/lm.upper.lid.txt --only-source --destdir $target_dir --thresholdsrc 2 --padding-factor 1 --dict-only
...
python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --workers 4 --dataset-impl mmap --trainpref $target_dir/phones.txt --only-source --destdir $target_dir/phones --thresholdsrc $min_phones --padding-factor 1 --dict-only
...
python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --workers 4 --dataset-impl mmap --trainpref $target_dir/phones/lm.phones.filtered.txt --workers 70 --only-source --destdir $target_dir/phones --srcdict $target_dir/phones/dict.phn.txt
...
echo "kaldi1"
lg=$lg python $FAIRSEQ_ROOT/examples/speech_recognition/kaldi/kaldi_initializer.py kaldi_root=$KALDI_ROOT fst_dir=$PWD/$target_dir/fst/phn_to_words_sil lm_arpa=$PWD/$target_dir/kenlm.wrd.o40003.arpa wav2letter_lexicon=$PWD/$target_dir/lexicon_filtered.lst data_dir=$PWD/$target_dir/phones in_labels=phn "blank_symbol='<SIL>'"
echo "kaldi2"
lg=$lg python $FAIRSEQ_ROOT/examples/speech_recognition/kaldi/kaldi_initializer.py kaldi_root=$KALDI_ROOT fst_dir=$PWD/$target_dir/fst/phn_to_words lm_arpa=$PWD/$target_dir/kenlm.wrd.o40003.arpa wav2letter_lexicon=$PWD/$target_dir/lexicon_filtered.lst data_dir=$PWD/$target_dir/phones in_labels=phn
...
lg=$lg python $FAIRSEQ_ROOT/examples/speech_recognition/kaldi/kaldi_initializer.py kaldi_root=$KALDI_ROOT fst_dir=$PWD/$target_dir/fst/phn_to_phn_sil lm_arpa=$PWD/$target_dir/phones/lm.phones.filtered.06.arpa data_dir=$PWD/$target_dir/phones in_labels=phn "blank_symbol='<SIL>'"
